...
public class SongListArrayAdapter extends ArrayAdapter<Song> implements Filterable{
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		if(song.getCover() != null){
	     albumCover.setImageBitmap(song.getCover()); /// TODO replace with Picasso --> rid of the bitmap in the model
		} else {
		 albumCover.setImageResource(R.drawable.img_cover); // here first display the image in the ImageBiew and the update the cove in the data base
    ...
    private void getCover(final Song song){
        if(song.getCover() == null) {
			new AsyncTask<Void, Void, Void>() {
				...
				protected Void doInBackground(Void... params) {
					...
					songManager.saveCover(song, imageByteArray); // TODO this is a call to the data base...this should be on background,
					return null;
				}
			}.execute();
